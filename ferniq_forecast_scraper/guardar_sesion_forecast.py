...
TARGET_URL = "https://appfernfutures-test.zafirus.tech/app/forecast-vs-goal/level2"
SESSION_FILE = BASE_DIR / "sesion_forecast.json"
SESSION_STORAGE_FILE = BASE_DIR / "sesion_forecast_session_storage.json"
PERSISTENT_PROFILE_DIR = BASE_DIR / ".forecast_browser_profile"
...
def ensure_dashboard_ready(page: Page) -> None:
    ...
    if is_not_found_page(page) and open_forecast_from_home(page):
        return
    ...
    raise RuntimeError(
        "No se pudo verificar una sesion autenticada en Ferniq Forecast. "
        f"URL actual: {current_url}"
    )
...
def try_auto_login(page: Page, email: str, password: str) -> bool:
    ...
    password_candidates = [
        page.get_by_label(re.compile(r"password|contrasena|clave|contraseña", re.I)),
        page.get_by_placeholder(re.compile(r"password|contrasena|clave|contraseña", re.I)),
        page.locator("input[type='password']"),
    ]
    submit_candidates = [
    ...
    with sync_playwright() as p:
        ...
        if not auto_login_ok and not is_dashboard_ready(page) and allow_manual:
            ...
            raise RuntimeError(
                ...
                "Revisa FERNIQ_FORECAST_EMAIL/FERNIQ_FORECAST_PASSWORD o ejecuta guardar_sesion_forecast.py manualmente."
            )
